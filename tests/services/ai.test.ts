  ...
  describe("generateWeeklySummary", () => {
    ...
    test("should generate AI summary successfully", async () => {
      ...
      expect(result).toContain(
        "This week, the team focused on implementing 8 new features"
      );
    ...
    test("should return null when OpenAI API key is not configured", async () => {
      ...
      jest.resetModules();
      const {
        AIService: AIServiceWithoutKey,
      } = require("../../src/services/ai");
      const aiServiceWithoutKey = new AIServiceWithoutKey();

      const result = await aiServiceWithoutKey.generateWeeklySummary(
        mockSummaryData
      );
    ...
    test("should build proper prompt with all data", async () => {
      ...
      expect(prompt).toContain(
        "Feature: 8 commits, Fix: 4 commits, Docs: 3 commits"
      );
